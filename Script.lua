...
local function SetMotionBlurEnabled(value)
    ...
    if value then
        if not MotionBlurEffect then
            MotionBlurEffect = lighting:FindFirstChild("MourazxMotionBlur")
            if not MotionBlurEffect then
                MotionBlurEffect = Instance.new("BlurEffect")
                MotionBlurEffect.Name = "MourazxMotionBlur"
                MotionBlurEffect.Parent = lighting
            end
        end
        MotionBlurEffect.Size = math.clamp(tonumber(MotionBlurIntensity) or 12, 0, 32)
        MotionBlurEffect.Enabled = true
    elseif MotionBlurEffect then
        MotionBlurEffect.Enabled = false
    end
end

local function SetMotionBlurIntensity(value)
    MotionBlurIntensity = math.clamp(tonumber(value) or 12, 0, 32)
    if MotionBlurEffect and MotionBlurEnabled then
        MotionBlurEffect.Size = MotionBlurIntensity
    end
end
...
MainTab:Toggle({
    Title = "Motion Blur",
    Desc = "Ativa desfoque de movimento local",
    Value = false,
    Callback = function(value)
...
MainTab:Slider({
    ...
    Desc = "Regula a intensidade do desfoque de 0 a 32",
    Value = {
        Min = 0,
        Max = 32,
        Default = 12,
