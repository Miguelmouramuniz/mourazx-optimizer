...
local function IsCombatEffect(object)
    ...
    local keywords = {
        ...
        "fruit", "weapon", "blade", "gun", "pistol", "rocket",
        "bomb", "bow", "flame", "magma", "ice", "light", "dark",
        "quake", "dragon", "dough", "leopard", "portal", "buddha",
        "electric", "flash", "particle",
    }
...
local function IsNamedCombatVisual(object)
    local names = {}
    local current = object
    ...
    for _, keyword in ipairs({
        "vfx", "effect", "aura", "haki", "particle", "slash",
        "explosion", "impact", "skill", "ability", "fruit",
    }) do
        if string.find(combinedName, keyword, 1, true) then
            return true
        end
                ...
                pcall(function()
                    if object:IsA("BasePart") then
                        ...
                        if object:IsA("MeshPart") then
                            pcall(function()
                                PotatoSave(object, "RenderFidelity")
                                object.RenderFidelity = Enum.RenderFidelity.Performance
                            end)
                        end

                        -- Oculta somente partes que parecem ser VFX de fruta,
                        -- Haki, skill, slash ou explosão; não remove o mapa.
                        if IsNamedCombatVisual(object) then
                            PotatoSave(object, "LocalTransparencyModifier")
                            object.LocalTransparencyModifier = 1
                        end
                    elseif object:IsA("Model") then
                        pcall(function()
                            PotatoSave(object, "LevelOfDetail")
                            object.LevelOfDetail = Enum.ModelLevelOfDetail.Low
                        end)
...
local DestructiveCleanupActive = false

local function IsProtectedFromDestructiveCleanup(object)
    local players = game:GetService("Players")
    local localPlayer = players.LocalPlayer

    if localPlayer and localPlayer.Character
        and object:IsDescendantOf(localPlayer.Character) then
        return true
    end

    if object:IsA("BackpackItem")
        or object:FindFirstAncestorWhichIsA("BackpackItem") then
        return true
    end
...
local function ApplyDestructivePotatoCleanup()
    if DestructiveCleanupActive or not BeginOptimization() then
        return
    end
    ...
    task.defer(function()
        ...
        local ok, errorMessage = pcall(function()
            ...
            -- MaterialVariants locais são apenas dados visuais e serão
            -- recriados pelo jogo quando uma nova sessão for iniciada.
            ...
            -- Usa uma lista congelada para não interromper GetDescendants
            -- enquanto objetos são destruídos.
            local descendants = game:GetDescendants()
            for _, object in ipairs(descendants) do
                if not IsProtectedFromDestructiveCleanup(object) then
                    local removable = object:IsA("SurfaceAppearance")
                        or object:IsA("Clothing")
        ...
        DestructiveCleanupActive = ok
        EndOptimization()

        if ok then
            Notify("Limpeza destrutiva concluída: " .. tostring(removed) .. " objetos removidos. Reentre no servidor para restaurar.", 8)
        else
            Notify("Limpeza destrutiva interrompida: " .. tostring(errorMessage), 6)
        end
    end)
end
...
OptiTab:Button({
    ...
    Desc = "Perfil PvP: remove VFX de frutas, armas, espadas e Haki",
    Callback = function()
        ApplyPotatoGraphics()
    end,
})
...
OptiTab:Button({
    Title = "Destructive Potato Cleanup",
    Desc = "Remove VFX, roupas e materiais visuais; exige reconectar para restaurar",
    Callback = function()
        ApplyDestructivePotatoCleanup()
    end,
})
