...
local MotionBlurIntensity = 6
local MotionBlurTrail
local MotionBlurAttachments = {}
local MotionBlurConnection

local function GetMotionBlurCharacter()
    local players = game:GetService("Players")
    local localPlayer = players.LocalPlayer
    return localPlayer and localPlayer.Character
end

local function DestroyCharacterMotionBlur()
    if MotionBlurConnection then
        MotionBlurConnection:Disconnect()
        MotionBlurConnection = nil
    end

    if MotionBlurTrail then
        MotionBlurTrail:Destroy()
        MotionBlurTrail = nil
    end

    for _, attachment in ipairs(MotionBlurAttachments) do
        pcall(function()
            attachment:Destroy()
        end)
    end
    MotionBlurAttachments = {}
end

local function CreateCharacterMotionBlur()
    local character = GetMotionBlurCharacter()
    local root = character and character:FindFirstChild("HumanoidRootPart")
    if not root then
        return false
    end

    if MotionBlurTrail and MotionBlurTrail.Parent == root then
        return true
    end

    DestroyCharacterMotionBlur()

    local top = Instance.new("Attachment")
    top.Name = "MourazxBlurTop"
    top.Position = Vector3.new(0, 2.3, 0)
    top.Parent = root

    local bottom = Instance.new("Attachment")
    bottom.Name = "MourazxBlurBottom"
    bottom.Position = Vector3.new(0, -2.3, 0)
    bottom.Parent = root
    MotionBlurAttachments = {top, bottom}

    local trail = Instance.new("Trail")
    trail.Name = "MourazxCharacterMotionBlur"
    trail.Attachment0 = top
    trail.Attachment1 = bottom
    trail.Color = ColorSequence.new(
        Color3.fromRGB(255, 95, 105),
        Color3.fromRGB(255, 220, 220)
    )
    trail.Transparency = NumberSequence.new({
        NumberSequenceKeypoint.new(0, 0.82),
        NumberSequenceKeypoint.new(0.35, 0.91),
        NumberSequenceKeypoint.new(1, 1),
    })
    trail.Lifetime = math.clamp(MotionBlurIntensity / 35, 0.06, 0.34)
    ...
    trail.Enabled = false
    trail.Parent = root
    MotionBlurTrail = trail
    return true
end

local function SetMotionBlurEnabled(value)
    ...
    MotionBlurConnection = RunService.Heartbeat:Connect(function()
        ...
        if not MotionBlurTrail or not MotionBlurTrail.Parent then
            CreateCharacterMotionBlur()
        end

        local character = GetMotionBlurCharacter()
        local root = character and character:FindFirstChild("HumanoidRootPart")
        if root and MotionBlurTrail then
            local speed = root.AssemblyLinearVelocity.Magnitude
            -- O rastro só aparece em movimento; parado não há blur.
            MotionBlurTrail.Enabled = speed >= 5
        end
    end)
end

local function SetMotionBlurIntensity(value)
    MotionBlurIntensity = math.clamp(tonumber(value) or 6, 1, 12)
    if MotionBlurTrail then
        MotionBlurTrail.Lifetime = math.clamp(MotionBlurIntensity / 35, 0.06, 0.34)
    end
end
...
EffectsTab:Toggle({
    Title = "Character Motion Blur",
    Desc = "Rastro aplicado no personagem somente durante movimento",
    Value = false,
    Callback = function(value)
...
EffectsTab:Slider({
    ...
    Desc = "Regula o rastro do personagem de 1 a 12",
    Value = {
        Min = 1,
        Max = 12,
        Default = 6,
