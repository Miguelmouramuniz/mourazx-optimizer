...
local PingMonitorEnabled = false
local CacheCleanerEnabled = false

local function ReadPing()
    local ok, value = pcall(function()
        local stats = game:GetService("Stats")
        local network = stats.Network
        local serverStats = network.ServerStatsItem["Data Ping"]
        return math.floor(serverStats:GetValue() + 0.5)
    end)
...
local function StartPingMonitor()
    if PingMonitorEnabled then
        return
    end

    PingMonitorEnabled = true
    task.spawn(function()
        while PingMonitorEnabled do
            local ping = ReadPing()
            if ping then
                Notify("Ping atual: " .. tostring(ping) .. " ms", 3)
            end
            task.wait(5)
        end
    end)
...
local function StopPingMonitor()
    PingMonitorEnabled = false
end

local function StartCacheCleaner()
    if CacheCleanerEnabled then
        return
    end

    CacheCleanerEnabled = true
    task.spawn(function()
        while CacheCleanerEnabled do
            task.wait(300)
            if CacheCleanerEnabled then
                CleanClientMemory()
            end
...
local function StopCacheCleaner()
    CacheCleanerEnabled = false
end
...
OptiTab:Toggle({
    Title = "Ping Monitor / Optimizer",
    Desc = "Monitora a latência a cada 5 segundos; não altera o servidor",
    Value = false,
    Callback = function(value)
        if value then
            StartPingMonitor()
...
OptiTab:Toggle({
    ...
    Desc = "Executa coleta segura de memória Lua a cada 5 minutos",
    Value = false,
    Callback = function(value)
        if value then
            StartCacheCleaner()
